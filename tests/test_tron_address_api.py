...
async def test_get_address_info_endpoint(client):
    ...
    A case where the address does not yet exist in the database and is obtained via TronService.
    """
    address_info = {
        "balance": 1000000,
        "bandwidth": 5000,
    ...
    with patch.object(
        ...
        response = client.post(f"/api/tron/get-address-info?address={test_address}")
